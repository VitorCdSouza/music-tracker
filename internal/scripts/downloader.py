...
def main(creds_path, download_path, download_quality, playlist_name):
    ...
    for _, track_id_base62 in enumerate(track_ids, 1):
        try:
            ...
            artist_name = (
                track_proto.artist[0].name if track_proto.artist else "unknown artist"
            )
            ...
            # download
            stream_data = session.content_feeder().load(
                track_id_obj, quality, False, None
            )
            ...
            if stream_data and stream_data.input_stream:
                ...
                actual_stream = (
                    audio_stream.stream()
                    if hasattr(audio_stream, "stream")
                    else audio_stream
                )
                ...
                try:
                    ...
                    print(
                        f"{track_id_base62} - falha ao salvar metadados: {e}",
                        file=sys.stderr,
                    )
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(
            "comando: python3 downloader.py <creds_path> <download_path> <quality> <playlist_name>",
            file=sys.stderr,
        )
        sys.exit(1)
